...
In which situations may I add a uniqueness constraint to a distributed table?
-----------------------------------------------------------------------------

Citus is able to enforce a primary key or uniqueness constraint on columns only when one of the columns is the distribution column. In particular this means that if a single column constitutes the primary key then it has to be the distribution key as well.
