...
For the foreign keys between reference tables and local tables, any `referential actions <https://www.postgresql.org/docs/current/ddl-constraints.html#DDL-CONSTRAINTS-FK>`_ is allowed for foreign keys from local tables to reference tables.
